  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", padding: "10px 24px 0", borderBottom: `1px solid ${J.brass}22` }}>
        <JTab on={view === "ledger"} onClick={() => { setFlow(null); setView("ledger"); }}>LEDGER</JTab>
        <JTab on={view === "delve"} onClick={() => { setFlow(null); setView("delve"); }}>DELVE</JTab>
        <JTab on={view === "inv"} onClick={() => { setFlow(null); setView("inv"); }}>PARTY INVENTORY</JTab>
        {!spectator && <JTab on={view === "char"} onClick={() => { setFlow(null); setView("char"); }}>CHARACTER</JTab>}
        <JTab on={view === "board"} onClick={() => { setFlow(null); setView("board"); }}>LEADERBOARD</JTab>
        <JTab on={view === "log"} onClick={() => { setFlow(null); setView("log"); }}>LOG</JTab>
        {isDM && <JTab on={view === "docket"} onClick={() => { setFlow(null); setView("docket"); }}>DOCKET</JTab>}
      </div>
