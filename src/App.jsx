...
const SCALE = 1.20; // ~20% larger UI + font
...
export default function App() {
  ...
  if (!game) {
    return (
      <Shell session={session} onLeave={leave} game={null} muted={muted} onToggleMute={toggleMute} onOpenCodex={openCodex} onOpenMusic={openMusic} onOpenDowntime={openDowntime}>
  ...
  return (
    <Shell session={session} onLeave={leave} game={game} muted={muted} onToggleMute={toggleMute} onOpenCodex={openCodex} onOpenMusic={openMusic} onOpenDowntime={openDowntime} onOpenVehicles={() => setShowUpgrades(true)} onOpenSheet={mySheet ? () => window.open(mySheet, "_blank", "noopener") : null}>
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 14, flexWrap: "wrap" }}>
        <div style={{ ...panel, flex: 2, minWidth: 240 }}>
        ...
        <div style={{ ...panel, flex: 1.9, minWidth: 340, display: "flex", gap: 14 }}>
...
function Shell({ session, onLeave, game, children, muted, onToggleMute, onOpenCodex, onOpenMusic, onOpenDowntime, onOpenVehicles, onOpenSheet }) {
  return (
    <div style={{
...
function VehicleUpgrades({ onClose, resolve, owned, combat, isDM, onBuy, onSetHP, onRepair, onRoll }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", top: 0, left: 0, width: `${100 / SCALE}vw`, height: `${100 / SCALE}vh`, zIndex: 60, background: "rgba(4,8,10,0.82)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", fontFamily: "'EB Garamond', Georgia, serif" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ ...modalShell, width: "min(1240px, 97vw)", height: "min(80vh, 760px)" }}>
